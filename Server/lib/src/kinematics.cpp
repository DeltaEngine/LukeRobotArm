...
namespace lukeRobot
{
  namespace kinematics
  {
    ...
    Coordinates forwardKinematics(float armOneDegree, float armTwoDegree, float leadScrewDisplacement)
    {
      ...
      auto length = (ARM_ONE_LENGTH - ((LEAD_SCREW_DISTANCE + leadScrewDisplacement) * sin(20 * PI / 180)));
      calculated.x = length * cos(radians(armOneDegree)) + ARM_TWO_LENGTH * cos(radians(armOneDegree + armTwoDegree));
      calculated.y = length * sin(radians(armOneDegree)) + ARM_TWO_LENGTH * sin(radians(armOneDegree + armTwoDegree));
      calculated.z = currentZ(leadScrewDisplacement);
      return calculated;
    }

    JointParameters inverseKinematics(
        float requiredX, float requiredY, float requiredZ,
