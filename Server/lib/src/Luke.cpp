...
namespace lukeRobot
{
	...
	{
		_servo.servos.writeTwoBytesRegister(_servo.servoInfo[4].servoId, STSRegisters::TORQUE_LIMIT, gripTorque);
	...
	bool Luke::isRobotMoving()
	{
		if (_servo.gripperType == Constants::GripperType::ONE_DOF)
		{
			for (int i = 0; i < 2; i++)
			{
				if (_servo.servos.isMoving(_servo.servoInfo[0].servoId) || _servo.servos.isMoving(_servo.servoInfo[1].servoId) || _servo.servos.isMoving(_servo.servoInfo[2].servoId) || _servo.servos.isMoving(_servo.servoInfo[4].servoId))
					return true;
				delay(10);
			}
		}
		else if (_servo.gripperType == Constants::GripperType::TWO_DOF)
		{
			for (int i = 0; i < 2; i++)
			{
				if (_servo.servos.isMoving(_servo.servoInfo[0].servoId) || _servo.servos.isMoving(_servo.servoInfo[1].servoId) || _servo.servos.isMoving(_servo.servoInfo[2].servoId) || _servo.servos.isMoving(_servo.servoInfo[3].servoId) || _servo.servos.isMoving(_servo.servoInfo[4].servoId))
					return true;
				delay(10);
			}
		}
