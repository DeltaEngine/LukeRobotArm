...
namespace lukeRobot
{
	...
	bool Servo::begin(Constants::GripperType GripperType)
	{
		...
		for (int i = 0; i < 5; ++i)
		{
			...
			servos.writeRegister(servoInfo[i].servoId, STSRegisters::TARGET_ACCELERATION, TARGET_ACCELERATION);
			delay(SERVO_REGISTER_DELAY_MS);
			if (i == 0)
			{
				servos.writeRegister(servoInfo[i].servoId, STSRegisters::TARGET_ACCELERATION, servoOneAcceleration);
				delay(SERVO_REGISTER_DELAY_MS);
			}
			if (calibServos)
			{
	...
	bool Servo::calibrateServos()
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			servos.writeRegister(servoInfo[i].servoId, STSRegisters::WRITE_LOCK, 0);
			delay(SERVO_REGISTER_DELAY_MS);
			servos.writeRegister(servoInfo[i].servoId, STSRegisters::RESPONSE_DELAY, SERVO_RESPONSE_DELAY);
			delay(SERVO_REGISTER_DELAY_MS);
			servos.writeRegister(servoInfo[i].servoId, STSRegisters::MAXIMUM_TEMPERATURE, MAX_SERVO_TEMP_CELSIUS);
			delay(SERVO_REGISTER_DELAY_MS);
			servos.writeRegister(servoInfo[i].servoId, STSRegisters::TORQUE_PROTECTION_TH, PROTECTION_TORQUE_LIMIT);
			delay(SERVO_REGISTER_DELAY_MS);
			servos.writeTwoBytesRegister(servoInfo[i].servoId, STSRegisters::TORQUE_LIMIT, LIMIT_TORQUE_THRESHOLD);
			delay(SERVO_REGISTER_DELAY_MS);
			servos.writeTwoBytesRegister(servoInfo[i].servoId, STSRegisters::POSITION_CORRECTION, 0);
			delay(SERVO_REGISTER_DELAY_MS);
			servos.writeRegister(servoInfo[i].servoId, STSRegisters::OVERLOAD_TORQUE, OVERLOAD_TORQUE_LIMIT);
			delay(SERVO_REGISTER_DELAY_MS);
			servos.writeRegister(servoInfo[i].servoId, STSRegisters::TORQUE_PROTECTION_TIME, OVERLOAD_PROTECT_TIME);
			delay(SERVO_REGISTER_DELAY_MS);
			servos.writeRegister(servoInfo[i].servoId, STSRegisters::OVERCURRENT_TIME, OVER_CURRENT_TIME);
			delay(SERVO_REGISTER_DELAY_MS);
			...
			servos.writeRegister(servoInfo[i].servoId, STSRegisters::TORQUE_SWITCH, 0);
			delay(SERVO_REGISTER_DELAY_MS);
			servos.writeTwoBytesRegister(servoInfo[i].servoId, STSRegisters::POSITION_CORRECTION, servoInfo[i].offset);
			delay(SERVO_REGISTER_DELAY_MS);
			servos.writeTwoBytesRegister(servoInfo[i].servoId, STSRegisters::TORQUE_LIMIT, TARGET_TORQUE);
			delay(SERVO_REGISTER_DELAY_MS);
			servos.setMode(servoInfo[i].servoId, STSMode::POSITION);
			servos.writeRegister(servoInfo[i].servoId, STSRegisters::TORQUE_SWITCH, 1);
			delay(SERVO_REGISTER_DELAY_MS);
			...
			else if (i == 0 || i == 1)
			{
				if (i == 0)
				{
					servos.writeRegister(servoInfo[i].servoId, STSRegisters::MINIMUM_ANGLE, 0);
					delay(SERVO_REGISTER_DELAY_MS);
					servos.writeRegister(servoInfo[i].servoId, STSRegisters::MAXIMUM_ANGLE, 0);
					delay(SERVO_REGISTER_DELAY_MS);
					servos.writeRegister(servoInfo[i].servoId, STSRegisters::ANGULAR_RESOLUTION, 2);
					delay(SERVO_REGISTER_DELAY_MS);
				}
